...
Drupal.section = Drupal.section || {};

Drupal.section.splitters = [];

/**
 * Fix the height of all splitters to be the same as the items they are
 * splitting.
 */
Drupal.section.fixHeight = function() {
  for (i in Drupal.section.splitters) {
    Drupal.section.splitters[i].fixHeight();
  }
}

Drupal.behaviors.sectionAdmin = {
  attach: function(context) {
    // Show/hide layout manager button
    $('input#panels-section-toggle-layout:not(.panels-section-processed)', context)
      .addClass('panels-section-processed')
      .click(function() {
        $('.panel-section-admin')
          .toggleClass('panel-section-no-edit-layout')
          .toggleClass('panel-section-edit-layout');

        if ($('.panel-section-admin').hasClass('panel-section-edit-layout')) {
          $(this).val(Drupal.t('Hide layout designer'));
          Drupal.section.fixHeight();
        }
        else {
    ...
    $('div.panels-section-splitter:not(.panels-splitter-processed)')
      .addClass('panels-splitter-processed')
      .each(function() {
        Drupal.section.splitters.push(new Drupal.section.splitter($(this)));
      });
...
Drupal.section.splitter = function($splitter) {
  var splitter = this;
  ...
  function splitterStart(event) {
    ...
    if (splitter.left_width_type == splitter.right_width_type) {
      ...
      if (splitter.left_width_type != 'px') {
        ...
        splitter.max = splitter.right.width() + splitter.left.parent().width() -
          (splitter.left.siblings(':not(.panels-section-splitter)').length * 25) - 25;
      }
      else {
        ...
        if (splitter.right_id) {
          splitter.max = splitter.left.width() + splitter.right.parent().width() -
            (splitter.right.siblings(':not(.panels-section-splitter)').length * 25) - 25;
        }
        else {
          var subtract = 0;
          splitter.left.siblings(':not(.panels-section-splitter)').each(function() { subtract += $(this).width()});
    ...
    if (splitter.left_unit) {
      splitter.left_box = $('<div class="section-splitter-hover-box">&nbsp;</div>');
    ...
    if (splitter.right_unit) {
      splitter.right_box = $('<div class="section-splitter-hover-box"></div>');
  ...
  function splitterEnd(event) {
    ...
    Drupal.ajax['section-splitter-ajax'].options.data = {
    ...
    $('.panel-section-edit-layout').trigger('UpdateFlexibleSplitter');
  };

  this.getSizes = function() {
    splitter.left_width = $splitter.children('.panels-section-splitter-left-width').html();
    splitter.left_scale = $splitter.children('.panels-section-splitter-left-scale').html();
    splitter.left_width_type = $splitter.children('.panels-section-splitter-left-width-type').html();
    splitter.right_width = $splitter.children('.panels-section-splitter-right-width').html();
    splitter.right_scale = $splitter.children('.panels-section-splitter-right-scale').html();
    splitter.right_width_type = $splitter.children('.panels-section-splitter-right-width-type').html();
  };
  ...
  splitter.left_class = $splitter.children('.panels-section-splitter-left').html();
  splitter.left_id = $splitter.children('.panels-section-splitter-left-id').html();
  splitter.left = $(splitter.left_class);
  splitter.right_class = $splitter.children('.panels-section-splitter-right').html();
  splitter.right_id = $splitter.children('.panels-section-splitter-right-id').html();
...
$(function() {
   ...
   * height fixing.
   */
  Drupal.ajax.prototype.commands.section_fix_height = function(ajax, command, status) {
    Drupal.section.fixHeight();
  };
   ...
   */
  Drupal.ajax.prototype.commands.section_set_width = function(ajax, command, status) {
   ...
   * group.
   */
  Drupal.ajax.prototype.commands.section_fix_firstlast = function(ajax, data, status) {
  ...
  var element_settings = {
    url: Drupal.settings.section.resize,
  ...
  Drupal.ajax['section-splitter-ajax'] = new Drupal.ajax('section-splitter-ajax', $('.panel-section-admin').get(0), element_settings);

  // Prevent ajax goo from doing odd things to our layout.
  Drupal.ajax['section-splitter-ajax'].beforeSend = function() { };
  Drupal.ajax['section-splitter-ajax'].beforeSerialize = function() { };
